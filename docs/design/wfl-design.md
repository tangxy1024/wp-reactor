...
- SCOPE：`match<keys:window_spec> { steps [derive] } -> score(expr)` / `on each alias [where expr] -> score(expr)` / `-> score { ... }`
...
stage         = trigger_clause , { join_clause } ;
...
match_clause  = "match" , "<" , match_params , ">" , "{" , [ key_block ] , on_event_block , [ close_block ] , [ derive_block ] (* L2 未实现 *) , "}" , "->" , score_out ;
match_params  = [ field_ref , { "," , field_ref } ] , ":" , window_spec ;
each_clause   = "on" , "each" , IDENT , [ "where" , expr ] , "->" , score_out ;
...
- `derive` 块可选；若存在，位于 `on event/on close/and close` 之后。
